...
function getEventSourceNameBasedOnEvent({ event }) {
  ...
  if (event.traceContext) {
    ...
    if (!functionsExtensionVersion) {
      console.warn(
        "The environment variable 'FUNCTIONS_EXTENSION_VERSION' is not set. Only the function runtime '~3' is supported."
      );
      ...
      console.warn(
        "The function runtime '" +
          ...
          "' is not supported. Only '~3' and '~4' are supported."
      );
    }
  }
  if (
...
const parseCookie = (str) =>
  str
    ...
    .reduce((acc, v) => {
      ...
      acc[decodeURIComponent(v[0].trim().toLowerCase())] = decodeURIComponent(
        v[1].trim()
      );
      return acc;
    }, {});
